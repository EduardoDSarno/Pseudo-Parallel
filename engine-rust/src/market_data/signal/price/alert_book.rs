...
use crate::market_data::signal::price::alert::{
    AlertKey, ManualPriceAlert, ManualPriceDirection, PriceKey,
};
...
impl ManualPriceAlertEntry {
    ...
    fn _remove_subscriber(&mut self) -> bool {
        self.subscriber_count -= 1;
    ...
    pub fn _subscriber_count(&self) -> usize {
...
impl IndividualPriceAlertBook {
    ...
    ) -> &mut BTreeMap<PriceKey, ManualPriceAlertEntry> {
        match direction {
    ...
    fn _map_ref(
        &self,
        direction: ManualPriceDirection,
    ) -> &BTreeMap<PriceKey, ManualPriceAlertEntry> {
        match direction {
    ...
    pub fn _get(
        ...
        price_key: PriceKey,
        direction: ManualPriceDirection,
    ) -> Option<&ManualPriceAlert> {
        self._map_ref(direction)
            .get(&price_key)
    ...
    pub fn _get_entry(
        ...
        price_key: PriceKey,
        direction: ManualPriceDirection,
    ) -> Option<&ManualPriceAlertEntry> {
        self._map_ref(direction).get(&price_key)
    }
    ...
    pub fn _remove(
        ...
        let should_remove = match map.get_mut(&price_key) {
            Some(entry) => entry._remove_subscriber(),
            None => return None,
        };
    ...
    pub fn alerts_crossed_above(
        ...
        /* Range search gives only alerts crossed between previous and current price */
        ...
        self.above_map
            ...
            .map(|(_, entry)| entry.alert().clone())
            .collect()
    }

    pub fn alerts_crossed_below(
        &self,
        previous_price: f64,
        current_price: f64,
    ) -> Vec<ManualPriceAlert> {
        ...
        /* Down move is the same idea, but the range is current..previous */
        let Some(previous_key) = PriceKey::from_price(previous_price) else {
            return Vec::new();
        };
        let Some(current_key) = PriceKey::from_price(current_price) else {
...
impl AlertBook {
    ...
    pub fn _delete_alert(&mut self, key: AlertKey) -> Result<ManualPriceAlert, Box<dyn Error>> {
        match self.books.get_mut(&key.coin) {
            Some(book) => book
                ._remove(key.price_key, key.direction)
    ...
    pub fn _get_alert(&self, key: &AlertKey) -> Option<&ManualPriceAlert> {
        match self.books.get(&key.coin) {
            Some(book) => book._get(key.price_key, key.direction),
            None => None,
        }
    }
    ...
    pub fn alerts_crossed_above(
        ...
        self.books
            .get(&coin)
            .map(|book| book.alerts_crossed_above(previous_price, current_price))
            .unwrap_or_default()
    }
    ...
    pub fn alerts_crossed_below(
        ...
        self.books
            .get(&coin)
            .map(|book| book.alerts_crossed_below(previous_price, current_price))
            .unwrap_or_default()
    }

    /* Returns true if this alert key exists */
    pub fn _contains_key(&self, key: &AlertKey) -> bool {
        self._get_alert(key).is_some()
    }

    /* Returns how many subscribers share this alert key */
    pub fn _subscriber_count(&self, key: &AlertKey) -> Option<usize> {
        self.books
            .get(&key.coin)
            .and_then(|book| book._get_entry(key.price_key, key.direction))
            .map(|entry| entry._subscriber_count())
    }
    ...
    pub fn _book_for_coin_ref(&self, coin: Coins) -> Option<&IndividualPriceAlertBook> {
        self.books.get(&coin)
    }
}
